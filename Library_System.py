...
class Book():
    ...
    def __str__(self):
        #fill this in
        string ="Author: "+self.author+", Title: "+self.title+"."
        #print(string)
        return string
...
class Library:
    ...
    def add_book(self, book_inst):
        if (book_inst.author, book_inst.title) not in self.books:
            ...
            # fill in here d['mynewkey'] = 'mynewvalue'
...
f = open('list_of_books.txt').read().splitlines() #remove \n using splitlines

book_list = []
for book in f:
    #fill in here
    author = book[:book.index("-")] # author is written before the -
    author = author.strip()
    title = book[book.index("-")+1:] # title is written after the -
    title = title.strip()
    book_instance = Book(title,author)
